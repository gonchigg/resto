...
for _ in range(100):
    ...
    resto = Resto.load_resto(file="resto_vacio.json", cant_mesas=20)
    cola = Resto.load_cola(file="cola_vacia.json", cant_clients=0)
    # Registro de llegadas y levantadas
    cola.registro_de_llegadas = Resto.giveme_llegadas(
        resto.mesas, factor_de_cantidad=4, proporciones=[2, 2, 3]
    )  # Con 4 va bien
